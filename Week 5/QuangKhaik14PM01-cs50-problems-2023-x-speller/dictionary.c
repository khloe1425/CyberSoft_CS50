...
// Hash table
node **table; //may need to use calloc
...
bool load(const char *dictionary)
{
    ...
    {
        node *n = NULL; // Khai báo biến n và khởi tạo nó bằng NULL
        n = malloc(sizeof(node)); // Cấp phát bộ nhớ cho n
        if (n == NULL)
        {
            // Xử lý lỗi khi không thể cấp phát bộ nhớ
            free(word); // Đừng quên giải phóng bộ nhớ cho word nếu có lỗi
            return false;
        }
